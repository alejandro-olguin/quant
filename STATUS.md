...
| **Pactos como módulo plug-in de referencia** — primer módulo en archivo propio (`js/modules/pactos.js`): se registra con `registerModule`, declara su proveedor de datos (capacidad `data` del contrato) y **consulta la API por contexto en cada navegación** (`GET data/pactos.json?cartera=…&corte=…`) en vez del cache hidratado al login. El shell gana soporte de vistas asíncronas (estado de carga, banner de error si la API falla, token que descarta respuestas tardías) y el helper `esc()` para texto servido por API. Es el molde del primer módulo que se conectará a data real (PRD §16.3-A1, primer incremento aplicado) | ✅ Completo (15-07-2026) |
| PRD actualizado a v1.5 (revisión técnica del prototipo, §16: bugs, hardening y brechas de arquitectura) | ✅ En el repo |

Verificación: los 24 módulos y sus sub-tabs renderizan sin errores en CLP, UF y USD (barrido de render por cada módulo/tab/moneda en el preview con Node); la red confirma los 17 GET de hidratación a `data/*.json` → 200 más el GET por contexto de Pactos (`pactos.json?cartera=…&corte=…`); conversión validada (MM$ 6.920.850 ≈ 176.358 M UF ≈ 7.404 MM US$). **(15-07-2026)** barrido repetido tras la migración de Pactos: 171 vistas (módulo × tab × moneda) renderizan sin errores; verificados además el estado de carga, el banner de error con la API caída (503) y el descarte de respuestas tardías tras navegar.
