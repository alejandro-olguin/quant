...
| **Versión del documento** | 1.5 — incorpora la **revisión técnica del prototipo** (§16): hallazgos, deuda técnica y condiciones de hardening antes de conectar datos y usuarios reales |
| **Fecha** | Julio 2026 |
...
**Changelog**
- **v1.5 (jul 2026):** se agrega §16 **Deuda técnica y hardening del prototipo**, resultado de una revisión de código completa del MVP: bugs a corregir, brechas de seguridad a cerrar antes del multi-usuario (escape de HTML, aislamiento de apps embebidas), y los dos incrementos que convierten la arquitectura de plug-in de narrativa en mecanismo (módulos en archivos propios y migración completa a `DataSource`).
...
## 16. Deuda técnica y hardening del prototipo — nuevo en v1.5

Resultado de una **revisión de código completa** del prototipo (shell `js/app.js`, cliente de datos `js/data.js`, `js/charts.js`, servidor de desarrollo, config de hosting, app embebida de ejemplo y los 18 recursos de datos), julio 2026. Veredicto general: el prototipo es sólido — cero dependencias aplicado con consistencia, documentación fiel a la realidad, y un diseño de hosting (config por ambiente en runtime, `no-store` en config/data, routing por hash) correcto para un SPA sin build. Los hallazgos siguientes son las **condiciones para pasar de prototipo a producto**, ordenados por impacto.

### 16.1 Bugs a corregir (independientes del backend)

| # | Hallazgo | Dónde | Severidad |
|---|---|---|---|
| B1 | **Login con carga de datos fallida deja la app rota**: `doLogin` captura el error de `ensureData()`, muestra un toast y aún así continúa — escribe la sesión, marca `authed` y llama `renderAll()`, que lanza `ReferenceError` (globales de datos no definidos). El flujo de `init()` sí retorna temprano; el de login necesita el mismo `return` en el `catch`. | `js/app.js` (`doLogin`) | Alta |
| B2 | **Datos publicados como globales sueltos en `window`**: `Object.assign(window, bundle)` funciona hoy, pero cualquier recurso futuro cuya clave colisione con una propiedad nativa de `window` (`name`, `status`, `length`, `top`, `history`…) se corrompe en silencio. Hidratar en un namespace único (p.ej. `window.QUANT_DATA` o cache interno) y que `DataSource` lea de ahí. | `js/data.js` (`loadAllData`) | Media |
| B3 | **Export CSV sin escape**: los campos entrecomillados no duplican comillas internas y los no entrecomillados se rompen con `;`. Agregar un `csvField()`. | `js/app.js` (`doExport`) | Baja |
| B4 | **Robustez de gráficos con data real**: `lineChart`/`sparkline` dividen por `points.length − 1` (NaN con serie de 1 punto) y `stackedBars` hace `Math.max(...)` sobre arreglos posiblemente vacíos. Hoy la data es controlada; con API real se necesitan guardas. | `js/charts.js` | Baja |

### 16.2 Seguridad — cerrar antes de conectar datos y usuarios reales

| # | Hallazgo | Implicancia | ¿Bloqueante? |
|---|---|---|---|
| S1 | **Inyección de HTML sistémica**: todas las vistas se construyen con `innerHTML` y literales de template, sin helper de escape. Ya hay tres entradas controladas por el usuario: nombres/descripciones de vistas personales (persisten en localStorage y se re-renderizan en el sidebar — XSS persistente), propuestas de la comunidad (`titulo`/`detalle`/`owner` — diseñadas para ser **datos compartidos** en producción, es decir XSS almacenado multi-usuario) y valores de búsqueda reflejados en atributos `value="…"`. Introducir un `esc()` y aplicarlo a todo string ingresado por usuario o servido por API. **(v1.5)** El helper `esc()` ya existe en el shell y el módulo Pactos lo usa en todo texto servido por API; queda pendiente aplicarlo al resto de las vistas y a las entradas del usuario (vistas personales, propuestas). | Es el patrón que hoy es inocuo con data sintética y catastrófico con backend multi-usuario. Es barato de corregir mientras el código vive en un archivo. | **Sí — antes de habilitar el builder/propuestas con persistencia compartida** (se suma a la pregunta abierta #16) |
| S2 | **Sandbox del iframe de módulos `app` es auto-derrotable**: `sandbox="allow-scripts allow-same-origin"` en un iframe del mismo origen no aísla nada (la app embebida alcanza `window.parent`). El contrato de plug-in debe documentar que el aislamiento real exige que las apps de terceros vivan en **otro origen** (como será en producción). | Contrato de integración de módulos `app` (§6.3) | Sí — antes de integrar apps de equipos externos |
| S3 | **Servidor de desarrollo**: el chequeo de prefijo (`startsWith(ROOT)`) admite directorios hermanos con el mismo prefijo, y sirve `.git/*` y configuración local. Solo afecta desarrollo; fix de una línea (`ROOT + path.sep` + lista de exclusión). | `.claude/serve.js` | No |

### 16.3 Brechas de arquitectura — convertir la narrativa en mecanismo

- **A1 — El modelo de plug-in está declarado pero parcialmente ejecutado.** El lado shell cumple (sidebar, ⌘K, galería y deep-links descubren los módulos desde el contrato), pero los 24 módulos viven en el mismo `js/app.js` (~3.200 líneas), `registerModule` tiene un solo consumidor (Laboratorio ESG) y solo 6 módulos consumen `DataSource`; los demás leen globales directamente. **Incrementos siguientes:** (1) separar las vistas en `js/modules/*.js` que llamen `registerModule`, (2) completar la migración de todas las vistas al proveedor — recién entonces "en producción se cambia el proveedor sin tocar el módulo" es verdad para todo el portal. **(v1.5) Primer incremento aplicado:** **Pactos (Repos)** migró a `js/modules/pactos.js` como **módulo plug-in de referencia** — se registra con `registerModule`, declara su proveedor propio (capacidad `data` del contrato) y consulta la API **por contexto en cada navegación** (cartera y corte como querystring) en vez del cache hidratado al login; el shell ganó soporte de vistas asíncronas (estado de carga, banner de error, descarte de respuestas tardías). Será el primer módulo conectado a data real; los demás siguen su molde.
- **A2 — El selector de fecha de corte afirma lo que la data no tiene.** Cambiar "Corte" re-renderiza y actualiza las etiquetas, pero la data no cambia; en un portal cuyo pitch es confiabilidad del dato, el demo no debe mentir. El contrato de API (`QuantAPI.get(recurso, params)`) ya anticipa el filtro por contexto; mientras el backend no filtre, fijar el selector o etiquetar "(demo: data única)".

### 16.4 Housekeeping

- Bytecode Python versionado en git (`__pycache__/*.pyc` bajo `docs/reference/cashflows-api/`): agregar `__pycache__/` y `*.pyc` a `.gitignore` y removerlos del índice.
- Restos de refactor en `js/app.js`: bloques `{ … }` huérfanos en `vCalce`/`vTsa`/`vValorizacion`, parámetro `f` sin uso en `politicaLimiteRows`, y tres paletas de colores casi idénticas que deberían ser una constante.
- **Accesibilidad**: las filas clickeables (`<tr>`/`<div>` con `data-action`) no son alcanzables por teclado — usar botones reales o `tabindex` + `role` + manejo de Enter. Los tooltips `<title>` en SVG y los botones semánticos del resto del shell ya están bien.

### 16.5 Prioridad sugerida

1. B1 (bug de 2 líneas que rompe la app en un escenario plausible).
2. S1 — helper `esc()` sobre strings de usuario, antes de que exista cualquier persistencia compartida.
3. B2 — namespace del cache de datos.
4. Housekeeping de git (`.pyc`).
5. A1 como siguiente incremento arquitectónico: archivos por módulo + migración completa a `DataSource`.

---

*Documento vivo. v1.2 refleja la incorporación de los ejercicios recurrentes de la mesa; v1.5 agrega la revisión técnica del prototipo (§16); revisar con Inversiones, Tesorería, Actuarial, TI/Arquitectura, Datos, Diseño y Compliance.*
